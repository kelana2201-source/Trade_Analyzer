/* Extracted App Logic - Enterprise Modular Layer (State, Errors, WS Heartbeat, Telemetry) */

// ═══════════════════════════════════════════════════════════
//  1. SINGLE UNIFIED APP STATE (AppState) & ERROR HANDLERS
// ═══════════════════════════════════════════════════════════
const AppState = {
  symbol: 'XAUUSD',
  prices: {
...
function initGlobalErrorHandlers() {
  window.onerror = function (message, source, lineno, colno, error) {
    ...
    if (typeof addLog === 'function') addLog(`[GLOBAL ERROR] ${errDetail}`, 'error');
    if (typeof setSystemStatus === 'function') setSystemStatus('ai', 'warn', 'Recovered');
    const el = document.getElementById('runtimeErrorsEl');
    if (el) { el.textContent = String(AppState.telemetry.errorsCount); el.className = 'telemetry-value text-amber-400 font-bold'; }
    return true; // Mencegah console spam dan menjaga antarmuka tetap stabil
  };

  window.addEventListener('unhandledrejection', function (event) {
    const reason = event.reason?.message || String(event.reason || 'Unhandled Promise Rejection');
    AppState.telemetry.errorsCount++;
    if (typeof addLog === 'function') addLog(`[ASYNC ERROR] ${reason}`, 'error');
    if (typeof setSystemStatus === 'function') setSystemStatus('ai', 'warn', 'Recovered');
    const el = document.getElementById('runtimeErrorsEl');
    if (el) { el.textContent = String(AppState.telemetry.errorsCount); el.className = 'telemetry-value text-amber-400 font-bold'; }
  });
...
function startWsHeartbeatMonitor() {
  ...
  wsHeartbeatTimer = setInterval(() => {
    ...
    if (elapsedSec > 25) {
      AppState.network.heartbeatStatus = 'SILENT';
      if (el) { el.textContent = `SILENT · ${elapsedSec}s`; el.className = 'telemetry-value text-amber-400 font-bold'; }
      addLog(`[WS HEARTBEAT] Tidak ada frame harga selama ${elapsedSec}d. Memulai koneksi ulang...`, 'error');
      closePriceSocket();
...
function schedulePriceSocketReconnect() {
  ...
  // Exponential Backoff dengan Jitter hingga maksimal 60 detik
  const delay = Math.min(60000, Math.round(1000 * Math.pow(1.8, priceSocketRetries) + Math.random() * 500));
  priceSocketRetries += 1;
  AppState.network.reconnectAttempts = priceSocketRetries;
  AppState.telemetry.wsReconnects++;
  const elRec = document.getElementById('wsReconnectsEl');
  if (elRec) { elRec.textContent = String(AppState.telemetry.wsReconnects); elRec.className = 'telemetry-value text-cyan-400'; }

  addLog(`[WS BACKOFF] Menjadwalkan koneksi ulang (#${priceSocketRetries}) dalam ${(delay / 1000).toFixed(1)} detik...`, 'info');
...
function connectPriceSocket() {
  ...
  try {
    ...
    sock.onmessage = function (ev) {
      AppState.network.lastHeartbeatTime = Date.now();
      const el = document.getElementById('wsHeartbeatEl');
      if (el) { el.textContent = 'OK · 0s'; el.className = 'telemetry-value text-emerald-400'; }
      let msg;
...
(function () {
  ...
  function updateTelemetryDashboard() {
    const ohlcCount = (OHLC_CACHE.M15?.length || 0) + (OHLC_CACHE.H1?.length || 0);
    const ohlcEl = document.getElementById('ohlcCacheEl');
    if (ohlcEl) ohlcEl.textContent = `${ohlcCount} bars`;

    const errEl = document.getElementById('runtimeErrorsEl');
    if (errEl) {
      errEl.textContent = String(AppState.telemetry.errorsCount);
      if (AppState.telemetry.errorsCount > 0) errEl.className = 'telemetry-value text-amber-400 font-bold';
    }

    const recEl = document.getElementById('wsReconnectsEl');
    if (recEl) {
      recEl.textContent = String(AppState.telemetry.wsReconnects);
      if (AppState.telemetry.wsReconnects > 0) recEl.className = 'telemetry-value text-cyan-400 font-bold';
    }
  }

  function runSystemDiagnostics() {
    showToast('Menjalankan Diagnostik Sistem Menyeluruh...', 'info');
    addLog('[DIAGNOSTICS] Memulai pengujian mandiri subsistem terminal...', 'info');
    ...
    setTimeout(() => {
      ...
      const wsOk = connected || livePriceVerified;
      checks.push({
      ...
      const barCount = (OHLC_CACHE.M15?.length || 0) + (OHLC_CACHE.H1?.length || 0);
      checks.push({
